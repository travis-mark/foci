(ns calm.foci
  (:require [com.biffweb :as biff]
            [calm.foci.feat.app :as app]
            [calm.foci.feat.auth :as auth]
            [calm.foci.feat.home :as home]
            [calm.foci.feat.worker :as worker]
            [calm.foci.schema :refer [malli-opts]]
...
(defn on-save [sys]
  ...
  (biff/eval-files! sys)
  (generate-assets! sys)
  (test/run-all-tests #"calm.foci.test.*"))
...
(def components
  [biff/use-config
   ...
   biff/use-chime
   (biff/use-when
    :calm.foci/enable-beholder
    biff/use-beholder)])

(defn start []
  (biff/start-system
   {:calm.foci/chat-clients (atom #{})
